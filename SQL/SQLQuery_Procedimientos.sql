...
BEGIN
    BEGIN TRY
        ...
        -- Cambiar el estado del plan de entrenamiento a 0 (eliminaci�n l�gica)
        UPDATE PlanEntrenamiento
        SET estado = 0
        WHERE id_plan = @id_plan;
...
BEGIN
    BEGIN TRY
        -- Verificar si existe el plan de entrenamiento y ha sido eliminado l�gicamente
        IF NOT EXISTS (SELECT 1 FROM PlanEntrenamiento WHERE id_plan = @id_plan AND estado = 0)
        BEGIN
            SET @respuesta = 0;
            SET @mensaje = 'El plan de entrenamiento no existe o ya est� activo.';
            RETURN;
        END
...
BEGIN
    BEGIN TRY
        -- Insertar la relaci�n entre el plan y el ejercicio
...
BEGIN
    BEGIN TRY
        -- Insertar la relaci�n entre el usuario y el plan
...
begin
     ...
     -- Eliminaci�n l�gica, se actualiza el campo 'estado' a 0 (inactivo)
...
begin
     ...
     -- Restauracion l�gica, se actualiza el campo 'estado' a 1 (activo)
...
BEGIN
    ...
    -- Obtener la �ltima fecha de pago y la duraci�n de la membres�a del alumno
    ...
    -- Calcular la fecha hasta d�nde alcanza la membres�a
...
BEGIN
   SET NOCOUNT ON;
    ...
    -- CTE para obtener la �ltima fecha de pago del alumno y la duraci�n de su membres�a
    ;WITH UltimaFecha AS (
        SELECT 
            a.id_alumno,
            m.duracion,
    ...
    FechasAdeudadas AS (
        -- Generar fechas de pago a partir de la �ltima fecha de pago
        SELECT 
            u.id_alumno,
    ...
    WHERE 
        -- Solo fechas vencidas o la del pr�ximo pago dentro del mes actual
